...
#  Define a Functional Graph of layers
...
block4Model = keras.Model(block_3_output, outputs, name="toy_resnetb4")


#  Retreive and prepare Training Data
...
# Call fit on the Models. Note: no training data for some
model.fit(x_train[:1000], y_train[:1000], batch_size=64, epochs=1, validation_split=0.2)

# Cannot call fit on the Block Models ONLY because there is no Training Data that has
# the right data/output size,
#block1Model.fit(x_train[:1000], y_train[:1000], batch_size=64, epochs=1, validation_split=0.2)

# Run Predictions/Processing
...
#  Check that the Complete Model and the Block Models
